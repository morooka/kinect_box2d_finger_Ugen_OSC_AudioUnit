...
bool ofxFingerDetector::findFingers (ofxCvBlob blob)
{
    ...
    for(int i=blob.nPts-k; i<blob.nPts; i++){        //トラッキングできてないところを補足トラッキング
        counter = i-(blob.nPts-k);
        
		v1.set(blob.pts[i].x-blob.pts[i-k].x, blob.pts[i].y-blob.pts[i-k].y);
		v2.set(blob.pts[i].x-blob.pts[counter].x, blob.pts[i].y-blob.pts[counter].y);
        
		v1D.set(blob.pts[i].x-blob.pts[i-k].x ,blob.pts[i].y-blob.pts[i-k].y, 0);
		v2D.set(blob.pts[i].x-blob.pts[counter].x ,blob.pts[i].y-blob.pts[counter].y, 0);
        
		vxv = v1D.cross(v2D);
		v1.normalize();
		v2.normalize();
        
        teta=v1.angle(v2);   //ここで35個前のポイントとの角度を求めている
        
		//control conditions
		if(fabs(teta) < 40)     //fabsは浮動小数点の絶対値を返す. もしteta(角度が)±40度以内の場合
		{	//pik?
			if(vxv.z > 0)     //ここをコメントアウトすると指の間も検出する
			{
                // if (ofDist(fingerPoints[i].x, fingerPoints[i].y,fingerPoints[i+1].x,fingerPoints[i+1].y) > 1) {
    ...
    for(int i=0; i<k; i++){        //トラッキングできてないところを補足トラッキング
		v1.set(blob.pts[i].x-blob.pts[blob.nPts-k+i].x, blob.pts[i].y-blob.pts[blob.nPts-k+i].y);
		v2.set(blob.pts[i].x-blob.pts[i+k].x, blob.pts[i].y-blob.pts[i+k].y);
        
		v1D.set(blob.pts[i].x-blob.pts[blob.nPts-k+i].x ,blob.pts[i].y-blob.pts[blob.nPts-k+i].y, 0);
        ...
        teta=v1.angle(v2);   //ここで35*2個前のポイントとの角度を求めている
